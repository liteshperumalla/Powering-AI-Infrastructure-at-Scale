...
class Assessment(Document):
    ...
    user_id: Annotated[str, Indexed()]  # ✅ Indexed for filtering by user
    ...
    # Assessment state
    status: Annotated[AssessmentStatus, Indexed()] = AssessmentStatus.DRAFT  # ✅ Indexed for filtering
    priority: Annotated[Priority, Indexed()] = Priority.MEDIUM  # ✅ Indexed for filtering
    ...
    experiments: list[Dict[str, Any]] = Field(
        ...
        description="A/B testing experiments for infrastructure changes"
    )
    ...
    # Timestamps
    created_at: Annotated[datetime, Indexed()] = Field(default_factory=datetime.utcnow, description="Assessment creation timestamp")  # ✅ Indexed for sorting
    ...
    class Settings:
        ...
        # Note: Indexes are managed by database_optimization.py to avoid conflicts during init
        # indexes = []
